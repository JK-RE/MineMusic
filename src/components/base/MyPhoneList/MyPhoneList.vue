...
export default {
  ...
  watch: {
    ...
    scrollY(newY) {
      ...
      for (let i = 0; i < leftListHeight.length - 1; i++) {
        ...
        if (-newY >= height1 && -newY < height2) {
          this.currentIndex = i
          this.diff = height2 + newY // 离下一区域的距离
          return
        }
      }
    ...
    diff(newVal) {
      ...
      this.$refs.fixedTitleRef.style.transform = `translate3d(0, ${fixedTop}px, 0)` // 实时改变滚动距离
    }
  },
  methods: {
    // 对父组件提供的刷新 better-scroll 方法
    refresh() {
    ...
    onShortcutTouchstart(e) {
      ...
      let nowTouch = e.touches[0] // 获取触摸对象
      this.touch.y1 = nowTouch.pageY

      let nowIndex = myDOM.customAttribute(e.target, 'index') // 获取相应的index值
      this.touch.nowIndex = nowIndex

      this._scrollTo(nowIndex) // 左侧列表相应滚动
    },
  ...
  computed: {
    ...
    shortcut() {
      return this.data.map((group) => {
        return group.title.substr(0, 1) // 截取每个title的第一个字符，返回一个新data
      })
    },
    // 返回字母列表
    fixedTitle() {
